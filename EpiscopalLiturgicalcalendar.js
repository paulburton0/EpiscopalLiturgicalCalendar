// This script populates a Google Calendar with events throughout the year 
// on the Episcopal Liturgical Calendar. To populate your calendar, create
// a new calendar in Google Calendar, and copy the Calendar ID (Calendar
// Settings > Calendar Details > Calendar Address > Calendar ID). Then
// paste the Calendar ID here (below):
var calendar = '<YOUR CALENDAR ID HERE>';
...
// Now click Run (above, in the menu bar), and then click populateCalendar
// It will take a few minutes to create all of the events on your calendar.
...
function calcEaster(year){
  ...
  return dayOfYear(month-1, date, year);
}

function extractMonth(dayOfYear, year){
   var total = dayOfYear, mctr = 0;  
   while(total > daysInMonth(mctr, year)){
     total -= daysInMonth(mctr, year);
     mctr++;       
   }
   return mctr;
}
...
function extractDate(dayOfYear, year){
   var total = dayOfYear, x;
   month = extractMonth (dayOfYear, year);
   for(x = 0; x < month; x++){
     total -= daysInMonth(x, year);
   }
   return total;
}
...
var fixedHolidays = new Array
  ("Christmas", "Dec 25", 
   "The Holy Name", "Jan 1", 
   "Epiphany", "Jan 6", 
   "Presentation of Jesus at the Temple (Candlemas)", "Feb 2", 
   "Holy Cross Day", "Sep 14",
   "Saint Michael and All Angels (Michaelmas)", "Sep 29",
   "All Saints", "Nov 1"
  );

if((annunciationDoy < easter - 7) && (annunciationDate.getDay() == 0)){
  annunciationDoy += 1;
  fixedHolidays.push("The Annunciation", "Mar 26");
}
else if((annunciationDoy >= easter - 7) && (annunciationDoy <= easter + 7)){
  easterCentricHolidays.push("The Annunciation", 8);
}else{
  fixedHolidays.push("The Annunciation", "Mar 25");
}
...
function calcEmberDays(){
  ...
  if(holyCrossDow < 2){
    ...
    emberDays.push("Ember Day", 'Sep ' + e1Da,
    "Ember Day", 'Sep ' + e2Da,
    "Ember Day", 'Sep ' + e3Da);
  }else{
  ...
  if(dec13Dow < 2){
    ...
    var e2Da = extractDate(ember2, year);
    var e3Da = extractDate(ember3, year);
    emberDays.push("Ember Day", 'Dec ' + e1Da,
    "Ember Day", 'Dec ' + e2Da,
    "Ember Day", 'Dec ' + e3Da);
  }else{
...
var advent = dayOfYear(11, fourthAdvent, year)
...
firstEpiphany = dayOfYear(0, epiphanySunday, year);
lastEpiphany = easter - 49;

function populateCalendar(){
  easterCount = easterCentricHolidays.length;
  for(x = 0; x < easterCount; x+=2){
    name = easterCentricHolidays[x];   
    offset = easterCentricHolidays[x+1];
    month = extractMonth(easter+offset, year);
    monthName = numToMonth(month);
    date = extractDate(easter+offset, year);
    eventDate = new Date(monthName + ' ' + date + ' ' + year);
    createEvent(calendar, name, eventDate);
  }
  
  fixedCount = fixedHolidays.length;
  
  for(x = 0; x < fixedCount; x+=2){
    name = fixedHolidays[x];
    fixedDate = fixedHolidays[x+1];
    eventDate = new Date(fixedDate + ' ' + year);
    createEvent(calendar, name, eventDate);
  }
  ...
  for(n = 5; n > 0; n--){
    if(n > 1){
      adventWeek = n-1;
      name = convertOrdinal(adventWeek) + ' Sunday of Advent';
    }else{
      name = "Christ the King";
    }
    month = extractMonth(advent, year);
    monthName = numToMonth(month);
    date = extractDate(advent, year);
    eventDate = new Date(monthName + ' ' + date + ' ' + year);
    createEvent(calendar, name, eventDate);
    advent -= 7;
  }
  ...
  while(properOffset+easter-7 < advent){
    name = convertOrdinal(sundayAfterPentecost) + ' Sunday after Pentecost (Proper ' + properNum + ')';
    month = extractMonth(easter+properOffset, year);
    monthName = numToMonth(month);
    date = extractDate(easter+properOffset, year);
    eventDate = new Date(monthName + ' ' + date + ' ' + year);
    createEvent(calendar, name, eventDate);
    properOffset += 7;
    properNum += 1;
  ...
  for(e = firstEpiphany; e <= lastEpiphany; e += 7){
    name = convertOrdinal(epiphanyNum) + ' Sunday after Epiphany';
    month = extractMonth(e, year);
    monthName = numToMonth(month);
    date = extractDate(e, year);
    eventDate = new Date(monthName + ' ' + date + ' ' + year);
    createEvent(calendar, name, eventDate);
    epiphanyNum ++;
  }
